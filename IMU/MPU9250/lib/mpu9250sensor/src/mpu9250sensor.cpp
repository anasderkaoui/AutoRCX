...
// If you want to recalibrate the IMU, set offsets to 0.0 and scales to 1.0, then do the calibration
// Magnetometer calibration (from 8-figure sweep)
constexpr double MAG_X_OFFSET = -4.3758;
constexpr double MAG_Y_OFFSET = 54.2583;
constexpr double MAG_Z_OFFSET = -5.9217;
constexpr double MAG_X_SCALE  = 34.4219;
constexpr double MAG_Y_SCALE  = 36.1438;
constexpr double MAG_Z_SCALE  = 32.8327;

// Accelerometer calibration (from six-face calibration)
constexpr double ACCEL_X_OFFSET = 0.57394;
constexpr double ACCEL_Y_OFFSET = -6.43432;
constexpr double ACCEL_Z_OFFSET = -0.63184;
constexpr double ACCEL_X_SCALE  = 1.00076;
constexpr double ACCEL_Y_SCALE  = 1.00147;
constexpr double ACCEL_Z_SCALE  = 1.00896;
...
double MPU9250Sensor::getAccelerationX() const
{
  ...
  return -1.0 * (accel_x_converted - ACCEL_X_OFFSET) / ACCEL_X_SCALE;
}
...
double MPU9250Sensor::getAccelerationY() const
{
  ...
  return -1.0 * (accel_y_converted - ACCEL_Y_OFFSET) / ACCEL_Y_SCALE;
}
...
double MPU9250Sensor::getAccelerationZ() const
{
  ...
  return -1.0 * (accel_z_converted - ACCEL_Z_OFFSET) / ACCEL_Z_SCALE;
}
...
void MPU9250Sensor::readMagnetometer()
{
    initMagnI2c();

    // 1. Check data ready (ST1 register, address 0x02)
    uint8_t st1 = i2cBus_->read(MAGN_ST1);
    if (!(st1 & 0x01)) {
        // Data not ready, don't update cache
        initImuI2c();
        return;
    }

    // 2. Read 6 bytes (X_L, X_H, Y_L, Y_H, Z_L, Z_H)
    uint8_t buf[6];
    for (int i = 0; i < 6; ++i) {
        buf[i] = i2cBus_->read(MAGN_XOUT_L + i);
    }

    // 3. Combine bytes: note LSB first
    ...
    mag_x_ = convertRawMagnetometerData(x);
    mag_y_ = convertRawMagnetometerData(y);
    mag_z_ = convertRawMagnetometerData(z);

    // Apply magnetometer calibration (hard and soft iron correction)
    mag_x_ = (mag_x_ - MAG_X_OFFSET) / MAG_X_SCALE;
    mag_y_ = (mag_y_ - MAG_Y_OFFSET) / MAG_Y_SCALE;
    mag_z_ = (mag_z_ - MAG_Z_OFFSET) / MAG_Z_SCALE;

    // 4. Optionally, read overflow/status (ST2 at 0x09)
    uint8_t st2 = i2cBus_->read(MAGN_ST2);
    if (st2 & 0x08) {
        std::cerr << "Magnetometer overflow!" << std::endl;
        // Don't use the measurement if overflow
        initImuI2c();
        return;
    }
...
double MPU9250Sensor::getMagneticFluxDensityX() const { return mag_x_; }
double MPU9250Sensor::getMagneticFluxDensityY() const { return mag_y_; }
double MPU9250Sensor::getMagneticFluxDensityZ() const { return mag_z_; }
