...
public:
    ...
    void spin() {
        ...
        while (ros::ok()) {
            ...
            if (steering_angle != 0.0 && linear_velocity != 0.0) {
                double turning_radius = linear_velocity / std::tan(steering_angle);
                delta_theta = linear_velocity * dt / turning_radius;
            }
...
private:
    ...
    void cmdVelCallback(const geometry_msgs::Twist::ConstPtr& msg) {
        // Ensure valid ranges to avoid NaN errors
        linear_velocity = std::clamp(msg->linear.x, -2.0, 2.0);
        steering_angle = (linear_velocity != 0.0) ? std::clamp(msg->angular.z, -3.114, 3.114) : 0.0;
    }
    ...
    void publishOdometry(const ros::Time& current_time) {
        ...
        // Velocity
        odom.twist.twist.linear.x = linear_velocity;
        odom.twist.twist.angular.z = (linear_velocity != 0.0) ? linear_velocity * std::tan(steering_angle) : 0.0;
